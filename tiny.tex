...
\item \textbf{Counit.} Because a lock on a context is just a function
  ...
  underlining operation in the type theory for $\natural$.
...
\item \textbf{Variable Usage.} To use a variable $x : A$ that is
  ...
  $\Gamma'$ contains a collection of locks
  $\ctxlocke{\lockn{L}_1}, \dots, \ctxlocke{\lockn{L}_n}$.
  % from left to right, and the variable $x$ lies between
  % $\ctxlocke{\lockn{L}_{i-1}$ and $\ctxlocke{\lockn{L}_i}$.
  \begin{mathpar}
    \inferrule*[left=var]{ \Gamma, x : A,
  ...
  it: typically these `keys' will not be stuck on the type $A$, unless
  $A$ is itself a variable.

\item \textbf{Calculating the Counit.}  On terms $a : A$, the term
  $\admkey{L}{t} a$ is calculated as follows. Suppose first that
  $t : \Tiny$ is a closed term. We induct on $a$ until we reach an
  instance of the variable rule. Suppose that, once reaching the
  variable, the locks on the context after $x : A$ (from left to
  right) are
  ...
  $\ctxlocke{\lockn{K}_i}$ occur between the use of
  $\admkeye{\lockn{L}}{t}$ and the variable, and placing
  $\varkeye{\lockn{L}}{t}$ to the left of the newly created
  $\ctxlocke{\lockn{K}_i}$.

  In the general case, when $t : \Tiny$ is not closed, the variables
  used in $t$ itself are now locked behind
  $\ctxlocke{\lockn{K}_n}, \dots, \ctxlocke{\lockn{K}_{i+1}}$ by the
  time we reach the variable $x$. We therefore need to (admissibly)
  apply the keys $\admkeye{\lockn{K}_n}{s_n} \dots
  \admkeye{\lockn{K}_{i+1}}{s_{i+1}}$ to the term $t$.
...
\begin{itemize}
\item \rulen{$\surd$-form/intro}: \mvrnote{Prose}
...
doesn't get used.
\item \rulen{$\surd$-elim}: Going under $\relim{i.r}$, the context remains
...
The first piece of notation is a compact version of the counit
function $\varepsilon_A : \rformu{A} \to A$ defined above, generalised
to allow us to skip past many locks in the context when extracting $A$.
\begin{definition}
  ...
  in $A$.
\end{definition}
(This cannot be internalised as a function, it is just
an admissible piece of syntax.)

For our second piece of notation, like ordinary $\lambda$-abstraction,
and as with any negative type former, the $\surd$-introduction rule is
right-invertible and so we may define terms of it via copattern
matching. We will write
\begin{align*}
  r(\ctxlock{L}, \ctxlock{K}) :\defeq q
  \quad \text{to mean} \quad
  r :\defeq \rintro{L}{\rintro{K}{q}}
\end{align*}
This combines nicely with ordinary copattern-style definitions of
functions. We write
\begin{align*}
  f(x,y,\ctxlock{L},z) :\defeq
  \quad \text{to mean} \quad
  f :\defeq \lambda x. \lambda y. \rintro{L}{\lambda z. 1}
\end{align*}
This syntax makes it easy to see at a glance which variables are
locked by what: the $x$ and $y$ here are to the left of $\ctxlock{L}$
in the list of arguments and so are locked.

Eliminating a term of $\rformu{A}$ in this style is a little peculiar.
To `evaluate' a lock we bind a new variable $i$ as in
$\surd$-elimination, but this variable is now in scope \emph{to the
  left} of the that lock. We write
\begin{align*}
  f(a(i), b(i), \rbindsym i., c)
  \quad \text{to mean} \quad
...
\begin{definition}
  ...
  \end{align*}
  \begin{align*}
    \eta_A(a, \ctxlock{L}, t) &:\defeq \varkey{L}{t} a \\
    \varepsilon_A(f) &:\defeq f(i, \rbindsym i.)
  \end{align*}
